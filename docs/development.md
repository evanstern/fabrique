...
```text
mongodb+srv://fabrique-app:<password>@<cluster-host>/?retryWrites=true&w=majority
```
...
- Missing `MONGO_URL`, `MONGO_DB`, `ANTHROPIC_API_KEY`, `FABRIQUE_PASSWORD`, or `FABRIQUE_AUTH_SECRET` will fail at runtime when the relevant boundary is reached.
